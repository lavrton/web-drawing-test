...
var HTMLStage = (function () {
    function HTMLStage() {
        ...
        this.stage = this.createStage(function () {
    ...
    HTMLStage.prototype.createStage = function (cb) {
...
var HTMLKineticStage = (function (_super) {
    ...
    HTMLKineticStage.prototype.createStage = function (cb) {
...
var PureKineticStage = (function (_super) {
    ...
    PureKineticStage.prototype.addSpecialNode = function () {
        ...
        spec.on("dragstart", function () {
            startX = spec.x();
            startY = spec.y();
            _this.layer._getIntersection = function () {
                return spec;
            };
        });
        ...
        spec.on("dragend", function () {
            _this.layer._getIntersection = oldFunc;
        });
        this.layer.add(spec);
    };
...
var HTMLFabricStage = (function (_super) {
    ...
    HTMLFabricStage.prototype.createStage = function (cb) {
...
var HTMLPhaserStage = (function (_super) {
    __extends(HTMLPhaserStage, _super);
    function HTMLPhaserStage() {
        _super.apply(this, arguments);
    }
    HTMLPhaserStage.prototype.createStage = function (cb) {
        var game = new Phaser.Game(STAGE_WIDTH, STAGE_HEIGHT, Phaser.AUTO, this.$stageContainer[0], {
            create: function () {
                setTimeout(cb);
            ...
            preload: function () {
                game.load.image('start', nodeImg.src);
            }
        });
        return game;
    };

    HTMLPhaserStage.prototype.render = function () {
        if (!this.stage.debug) {
            return;
        }
        this.connectors.forEach(function (c) {
            this.stage.debug.geom(c, '#cfffff');
        }.bind(this));
    };

    HTMLPhaserStage.prototype.updateConnectorPosition = function (connector) {
        var p1 = this.getNodePosition(connector.node1);
        var p2 = this.getNodePosition(connector.node2);
        connector.start.x = p1.x;
        connector.start.y = p1.y;
        connector.end.x = p2.x;
        connector.end.y = p2.y;
    };

    HTMLPhaserStage.prototype.addConnector = function (node1, node2) {
        var connector = new Phaser.Line(0, 0, 0, 0);
        connector.node1 = node1;
        connector.node2 = node2;
        return connector;
    };
    return HTMLPhaserStage;
})(HTMLStage);

var PurePhaserStage = (function (_super) {
    __extends(PurePhaserStage, _super);
    function PurePhaserStage() {
        _super.apply(this, arguments);
    }
    PurePhaserStage.prototype.render = function () {
        this.onNodeMove();
        _super.prototype.render.call(this);
        if (!this.stage.debug) {
            return;
        }
        this.stage.debug.geom(this.spec, 'red');
    };
    PurePhaserStage.prototype.addNode = function () {
        var node = this.stage.add.sprite(0, 0, 'start');
        this.nodes.push(node);

        // this.stage.add.sprite(0, 0, 'star');
        node.inputEnabled = true;

        //  Allow dragging - the 'true' parameter will make the sprite snap to the center
        node.input.enableDrag();
        node.events.onDragStart.add(function () {
            //this.stage.paused = false;
        });
        node.events.onDragStop.add(function () {
            // this.stage.paused = true;
        });
        return node;
    };

    PurePhaserStage.prototype.setNodePosition = function (node, pos) {
        if (typeof pos === "undefined") { pos = this.getRandomPosition(); }
        node.x = pos.x - nodeImg.width / 2;
        node.y = pos.y - nodeImg.height / 2;
    };

    PurePhaserStage.prototype.getNodePosition = function (node) {
        return {
            x: node.x + nodeImg.width / 2,
            y: node.y + nodeImg.height / 2
        };
    };

    PurePhaserStage.prototype.moveNode = function (node, dx, dy) {
        node.x += dx;
        node.y += dy;
    };

    PurePhaserStage.prototype.addSpecialNode = function () {
        var spec = new Phaser.Rectangle(0, 550, 800, 50);
        this.spec = spec;
    };
    return PurePhaserStage;
})(HTMLPhaserStage);
;
...
var HTMLEaselStage = (function (_super) {
    ...
    HTMLEaselStage.prototype.createStage = function (cb) {
        var _this = this;
        var canvas = $("<canvas width='" + STAGE_WIDTH + "' height='" + STAGE_HEIGHT + "'>").appendTo(this.$stageContainer);
...
nodeImg.onload = function () {
    ...
    new HTMLStage();
    $("<h3>Test 8 - Nodes: HTML, Connectors: Phaser</h3>").appendTo("body");
    new HTMLPhaserStage();
    $("<h3>Test 9 - Pure Phaser</h3>").appendTo("body");
    new PurePhaserStage();
};
